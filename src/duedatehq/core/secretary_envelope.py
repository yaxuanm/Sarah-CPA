...
def envelope_from_response(response: dict[str, Any]) -> dict[str, Any]:
    ...
    action = SecretaryAction(
        ...
        announce="Pull supporting material" if should_render else None,
...
def _summary_from_view(view: dict[str, Any]) -> str | None:
    ...
    if view.get("type") == "ClientListCard":
        ...
        return f"{total} clients are ready to review."
    if view.get("type") == "ListCard":
        items = data.get("items") if isinstance(data.get("items"), list) else []
        first = items[0] if items and isinstance(items[0], dict) else {}
        if first:
            return f"{len(items)} work items. The nearest one is {first.get('client_name') or 'the current client'}: {first.get('tax_type') or 'deadline item'}."
        return f"{len(items)} work items."
