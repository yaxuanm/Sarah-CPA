...
STATE_SOURCE_OVERRIDES: dict[str, dict[str, object]] = {
    "CA": {
    ...
    "TX": {
        "display_name": "Texas Comptroller News Releases",
        "default_url": "https://comptroller.texas.gov/about/media-center/news/index.php",
        "fetch_format": "html",
    },
    "NY": {
        "display_name": "New York Tax Department Press Office",
...
def official_source_registry() -> dict[str, SourceDefinition]:
    ...
    for state in STATE_CODES:
        ...
        registry[f"state_{state.lower()}"] = SourceDefinition(
            ...
            official=True,
            poll_frequency_minutes=60,
            display_name=str(override.get("display_name", f"{state} Department of Revenue")),
            default_url=str(override.get("default_url", f"https://www.{state.lower()}.gov/tax")),
            fetch_format=str(override.get("fetch_format", "html")),
        )
    return registry
