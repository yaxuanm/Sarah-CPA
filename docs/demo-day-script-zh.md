...
它不是一个更漂亮的日历，也不是一个报税软件。它解决的是更前置的问题：当 CPA 同时管理几十个客户、多个州和不断变化的规则时，她每天怎么知道先做什么、谁被卡住了、哪条变化真的影响客户。

产品里有两个入口：Dashboard 是结构化工作台；Ask 是自然语言入口。一个适合系统性查看，一个适合直接问“今天先做什么”或者“这条规则影响谁”。两者背后用的是同一套客户、规则和 deadline 数据。

## 1. Sarah 的处境

我们先从一个具体用户讲起。

Sarah Johnson 是一个在 Dallas 独立执业的 CPA。她管理 60 个客户，其中 23 个有跨州经营，全年有 400 多个联邦和州级 deadline。

她每周大概花 12 个小时维护这些日期和状态。问题是，这些时间不是在做税务判断，而是在维护“我到底什么时候该做什么”。她现在靠 Excel、Google Calendar、Email 和同行群拼出一个工作流，但没有一个地方真正负责结果。

## 2. 真实代价

这个问题平时只是麻烦，但一旦官方规则变化，就会变成风险。

比如 IRS 或州税局发布一条延期公告。Sarah 可能不是第一时间知道，等她从同行群里看到时，已经过去 48 小时。

然后她要回到 Excel 里逐个客户核对：谁在这个州？谁适用这个税种？谁的 deadline 需要变？

所以这不是“提醒不够好”的问题，而是小型 CPA firm 很难人工维护 50 州规则变化。

## 3. 产品定位

DueDateHQ 的核心对象不是 calendar 上的一个格子，而是一个 work item。

一个 work item 里面包含客户、税种、州、due date、状态、负责人、blocker、extension 和沟通记录。

所以用户看到的不是一堆日期，而是一个行动工作台。

Work now 是现在可以推进的事项；Blocked 是缺文件或缺确认，暂时推不动；Needs review 是官方规则变化，需要 CPA 判断；Archive 是已经处理完的事项。

接下来我用三个用户故事来展示。

## 4. User Story 1: Import

第一个故事是：Sarah 收到一份新的客户表，怎么把客户变更同步进 DueDateHQ。

现实里，CPA 的客户信息不是一次录完就永远不变。新客户会进来，旧客户的州足迹、联系人、税种也会更新。所以 import 不只是 onboarding，也是持续维护 portfolio 的入口。
...
所以我们的 AI 不只是一个聊天入口。它更像是把客户数据、官方规则和下一步行动连起来的工作流层。
